...
int last_back_pos = 0;
int front_pos_offset = 0;
int back_pos_offset = 0;
...
void setup()
{
  ...
  last_front_pos = analogRead(front_arm_pos_input_pin);
  last_back_pos = analogRead(back_arm_pos_input_pin);
}
...
void loop()
{
  ...
  Serial2ROS();
//  Serial2.print(front_arm_pos);
//  Serial2.print(",");
//  Serial2.println(back_arm_pos);
  ...
  else    //无控制源
  {
    ...
    if (watch_dog > 20)
    {
      ...
      digitalWrite(armMotor_enable_output_pin,LOW);//摆臂电机不使能
      OCR1C=128;
      OCR1B=128;
    }
  }
...
void  ReadAD(int pinFront,int pinBack)
{
  ...
  if (front_arm_pos - last_front_pos > 500)
  {
    front_pos_offset -= 1024;
  }
  if (front_arm_pos - last_front_pos < -500)
  {
    front_pos_offset += 1024;
  }
  if (back_arm_pos - last_back_pos > 500)
  {
    back_pos_offset -= 1024;
  }
  if (back_arm_pos - last_back_pos < -500)
  {
    back_pos_offset += 1024;
  ...
  last_front_pos = front_arm_pos;
  last_back_pos = back_arm_pos;

  front_arm_pos += front_pos_offset;
  back_arm_pos += back_pos_offset;
